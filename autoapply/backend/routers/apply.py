...
async def trigger_auto_apply(job_id: str, db: Session = Depends(get_db)):
    ...
    if tailored.resume_source == ResumeSource.local:
    ...
    elif tailored.resume_source == ResumeSource.google_doc:
        try:
        ...
        raise HTTPException(status_code=400, detail="Tailored resume data is invalid or in error state.")
